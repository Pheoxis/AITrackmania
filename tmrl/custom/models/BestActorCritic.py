...
class RCNNQFunction(nn.Module):
    ...
    def __init__(
            ...
            activation=nn.GELU, num_quantiles=3
    ):
        super().__init__()
        ...
        self.q_model_out = nn.Linear(self.conv_branch.mlp_out_size + 3, 1)
...
class SquashedActorRCNN(TorchActorModule):
    # domyślne wartości parametrów muszą się zgadzać
    def __init__(
            self, observation_space, action_space, rnn_size=128, rnn_len=2, mlp_branch_sizes=(256, 512, 256),
            activation=nn.GELU
...
class RCNNActorCritic(nn.Module):
    # domyślne wartości parametrów muszą się zgadzać
    def __init__(
            self, observation_space, action_space, rnn_size=128, rnn_len=2, mlp_branch_sizes=(256, 512, 256),
            activation=nn.GELU
