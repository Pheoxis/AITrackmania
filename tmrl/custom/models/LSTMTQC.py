...
class CNNModule(nn.Module):
    def __init__(self, grayscale: bool = cfg.GRAYSCALE, mlp_out_size: int = 128,
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d((4, 12))
        self.fc1 = nn.Linear(in_features=2976, out_features=mlp_out_size)
    ...
    def forward(self, x):
        for layer in self.conv_blocks:
            if isinstance(layer, nn.ModuleList):
                # Handle the residual block
                residual = x
                for sub_layer in layer:
                    residual = sub_layer(residual)
                x = x + residual  # Add the skip connection
            else:
                x = F.relu(layer(x))
        x = self.avg_pool(x)

        x = self.flatten(x) # 1 x 62 x 4 x 12
        x = self.fc1(x)
        x = self.dropout(x)
...
class QRCNNQFunction(nn.Module):
    ...
    def __init__(
            self, observation_space, action_space, rnn_size=192, rnn_len=1, mlp_branch_sizes=(192, 320, 128),
